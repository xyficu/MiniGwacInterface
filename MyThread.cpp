...
BEGIN_MESSAGE_MAP(CMyThread, CWinThread)
	...
	ON_THREAD_MESSAGE(WM_TAKESNAPSHOTASN, &CMyThread::TakeSnapshotAsn)
...
void CMyThread::TakeSnapshotAsn( WPARAM wParam, LPARAM lParam )
{
	m_pU9000Params->stTakeSnapshot.fileNameSu = 1;
	while (m_pU9000Params->stTakeSnapshot.countCrt < m_pU9000Params->stTakeSnapshot.countAsn)
	{
		CString fileName;
		fileName.Format("%s_%04u", m_pU9000Params->stTakeSnapshot.fileName, m_pU9000Params->stTakeSnapshot.fileNameSu);
		...
		if (0 == m_pU9000Params->stTakeSnapshot.status)
		{
			...
			m_pU9000Params->stTakeSnapshot.countCrt ++;
		}
		else
		{
			//m_pMainWnd->PostMessageA(WM_STOPTAKESNAPSHOTCONCPLT);
